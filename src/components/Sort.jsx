        ...
        React.useEffect(() => {
            const handleClickOutSide = (event) => {
                if (!event.composedPath().includes(sortUrl.current)) {
                    setOpen(false);
                }
                }
            ...
            </div>
        {
            ...
            <ul>
            {
            list.map((obj, i) => (<li key={i} onClick={() => onClickListItem(obj)} className={sort.sortProperty === obj.sortProperty ? 'active' : ''}>{obj.name}</li>))
            }
            </ul>
            </div>)
        }
